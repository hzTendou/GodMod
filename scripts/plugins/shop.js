        ...
        shopui.show(player).then(res => {
            ...
            blocksui.show(player).then(res => {
                if(res.selection == 0) {
                    ...
                    .title("§cBuy Brick")
                    .slider("§2Select amount", 1, 64, 10, 1)
                    brick.show(player).then(res => {
                      ...
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s brick_block ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                }
                if(res.selection == 1) {
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s stone ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s stonebrick ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s end_bricks ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s end_brick ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s log ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                ...
                if(res.selection == 6) {
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s glass ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                ...
                if(res.selection == 7) {
                    ...
                    brick.show(player).then(res => {
                      ...
                      player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                      if(player.hasTag("havemoney")){
                          player.runCommand(`give @s sand ${res.formValues[0]}`)
                          player.runCommand(`scoreboard players remove @s money ${bamo}`)
                          player.runCommand(`tag @s remove havemoney`)
                      }
                    })
                ...
                toolsui.show(player).then(res => {
                    if(res.selection == 0) {
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s chest ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                    ...
                    if(res.selection == 1) {
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s ender_pearl ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s fishing_rod ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s lantern ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s torch ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s cooked_beef ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                    ...
                    if(res.selection == 6) {
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s armor_stand ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
                    ...
                    if(res.selection == 7) {
                        ...
                        brick.show(player).then(res => {
                          ...
                          player.runCommand(`tag @s[scores={money=${bamo}..}] add havemoney`)
                          if(player.hasTag("havemoney")){
                              player.runCommand(`give @s oak_sign ${res.formValues[0]}`)
                              player.runCommand(`scoreboard players remove @s money ${bamo}`)
                              player.runCommand(`tag @s remove havemoney`)
                          }
                        })
