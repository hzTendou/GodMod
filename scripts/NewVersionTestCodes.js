   ...
   world.events.tick.subscribe(data => {
     for(let x=0; 32000 > x; x++){
       if(db.hasKey('codes', `command-${x}`)){
         ow.runCommand(db.getVar("codes",`command-${x}`))
       } else {
         break;
       }
     }
...
setTickInterval(() => {
  ...
  for(let player of world.getPlayers()){
    ...
    let z = player.location.z
        ...
        console.log(count)
        if(count <= 0){
          ow.runCommand(`summon tgm:chestuimarket "§l§9Public Market §8[-${player.name}-]" ${x} ${y} ${z}`)
          ow.runCommand(`scoreboard players set "${player.name}" chestGuiMarket 1`)
        }
  }
    } catch(e) {
      ow.runCommand(`say ${e}`)
       ...
       for(let player of world.getPlayers()){
         if(player.name == name){
           ...
           if(item?.typeId == 'minecraft:book' && item?.data == 6969){
             player.runCommand(`execute @s ~~~ tp @e[type=tgm:chestuimarket, name="${entity.nameTag}"] ^^1.35^0.5`)
           } else {
             player.runCommand(`execute @s ~~~ tp @e[type=tgm:chestuimarket, name="${entity.nameTag}"] ^^11^`)
           }
         }
       }
     }
      ...
      if(res.selection == 0){
        ...
        itemadd.show(playerd).then(res => {
              ...
              let item = inv1.getItem(parseInt(res.formValues[0]))
                ...
                let name = namep[1]
                ow.runCommand(`tag @e[type=tgm:database] add "PublicMarket,${item.typeId},${item.amount},${item.data},${res.formValues[1]},${name}"`)
